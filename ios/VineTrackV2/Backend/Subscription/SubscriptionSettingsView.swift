...
struct SubscriptionSettingsView: View {
    ...
    private var statusSection: some View {
        Section {
            HStack(spacing: 12) {
                Image(systemName: subscription.isSubscribed ? "checkmark.seal.fill" : "lock.fill")
                    .font(.title2)
                    .foregroundStyle(subscription.isSubscribed ? Color.green : Color.orange)
                    .frame(width: 36, height: 36)
                VStack(alignment: .leading, spacing: 2) {
                    Text(subscription.isSubscribed ? "Vineyard Tracker Pro" : "No active subscription")
                        .font(.headline)
                    Text(statusSubtitle)
                        .font(.caption)
